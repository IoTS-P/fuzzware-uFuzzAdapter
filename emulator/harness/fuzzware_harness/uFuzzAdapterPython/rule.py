from ... import globs
from ...utils import get_realpath, my_debug_log
...
import pickle
import re
# modify import and global variable begin
from .emulation_handler import EmulationHandler
            ...
            if action.a1.phaddr not in RULE.data_regs:
                ...
                if action.a1.type == '#R':
                    ...
                    if (len(a1_fifo) <= int(value2/8)):
                        ...
                        my_debug_log("[-] Not Acceptable Rule Action! Error: (fifo.size != size) or (to add unknown value). Action Reg: %s" % hex(action.a1.phaddr))
                        do_exit(-1)
...
def hardware_write_to_receive_buffer(data_reg, phaddr, value_queue, most_size=0):
    '''
    hardware write to data_reg's receive buffer.
    if most_size != 0, then the size of receive buffer will be limited.
    '''
    # add all the queue in user_r_fifo
    data_reg.user_r_fifo += value_queue
    # if most_size != 0, limit the size of receive buffer
    if most_size != 0:
        # if r_fifo is still not full, add some value from user_r_fifo
        r_fifo_len = len(data_reg.r_fifo)
        if r_fifo_len < most_size:
            data_reg.r_fifo += data_reg.user_r_fifo[:most_size-r_fifo_len]
            data_reg.user_r_fifo = data_reg.user_r_fifo[most_size-r_fifo_len:]
    # if most_size == 0, just add all the queue in r_fifo
    else:
        data_reg.r_fifo += data_reg.user_r_fifo
        data_reg.user_r_fifo = []
    # update r_size
    data_reg.r_size = len(data_reg.r_fifo) * 8
...
def hardware_write_to_receive_buffer_list(data_regs_list, value, most_size=0):
    '''
    ...
    for address in data_regs_list:
        ...
        my_debug_log("hardware_write_to_receive_buffer: {}, {}".format(hex(address), value))
        hardware_write_to_receive_buffer(data_reg, address, value, most_size)
...
def get_value_from_receive_buffer(data_reg, phaddr, size):
    ...
    if data_reg.r_size == 0:
        if nvic_get_active() == 0 or nvic_get_active() == 0xf:
            ...
            my_debug_log("[+] no data in 0x%x, need to exit." % phaddr)
            # do_exit(0)
    else:
        
        # my_debug_log("[+] get data from 0x%x, data: 0x%x" % (phaddr, value))
        pass
...
#------------------minipython modify begin------------------------#

def call_hardware_clear_r_fifo(dr_list):
    for data_reg in dr_list:
        RULE.regs[data_reg].value.user_r_fifo = []
...
def beginpointHook(uc, address, size, user_data):
    ...
    globs.config.fork_point_times = len(globs.user_input)>>2
    
    my_debug_log("beginpointHook")
    
    RULE.ghidra_emulate.recover_shm_and_add_dr_hook()
...
class RULE():
    ...
    def readNLPModelfromFile(ru, uc, path):
        ...
        def readInit(lines):
            ...
            for line in lines:
                ...
                if typ_0 == 'S': # status_reg
                    # just record the first type
                    ru.status_regs.add(phaddr)
    ...
    def configure(ru, uc, path,):
        ...
        from .emulation_handler import EmulationHandler
        ru.ghidra_emulate = EmulationHandler(globs.config, RULE.data_regs, RULE.status_regs)